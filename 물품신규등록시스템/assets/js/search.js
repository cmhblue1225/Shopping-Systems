...
function setupAutocomplete() {
    const autocompleteFields = [
        ...
        { inputId: 'codeFilter', listId: 'codeAutocomplete' },
...
function updateAutocompleteOptions() {
    ...
    allData.forEach(data => {
        ...
        if (data.items) {
            data.items.forEach(item => {
                ...
                if (item.code) options.code.add(item.code);
    ...
    window.autocompleteOptions = {
        ...
        codeFilter: Array.from(options.code),
...
function searchData() {
    const filters = {
        ...
        code: document.getElementById('codeFilter').value.toLowerCase().trim(),
    ...
    filteredData = allData.filter(data => {
        ...
        // 품목 관련 필터
        if (filters.vendor || filters.item || filters.code || filters.category || filters.warehouse) {
...
async function exportSelectedToExcel() {
    ...
    try {
        ...
        for (let i = 0; i < selectedItems.length; i++) {
            ...
            const ws = workbook.addWorksheet(`물품등록_${i + 1}`);
        ...
        saveAs(new Blob([buf], {type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"}), 
               `물품등록정보_${selectedItems.length}건_${new Date().toISOString().split('T')[0]}.xlsx`);
    } catch (error) {
...
async function generateExcelSheet(ws, data) {
    ...
    const sheet = [
        ...
        ['', '', '', '', '', '', '', '', '', '', '', ''],
        ['등록일자', '', '', '', '등록번호', '', '', '', '등록자', '', '', ''],
        [data.reqDate || '', '', '', '', data.reqNo || '', '', '', '', data.reqUser || '', '', '', ''],
        ['', '', '', '', '', '', '', '', '', '', '', ''],
        ['참조부서', '', refDept, '', '', '', '', '', '', '', '', ''],
        ['', '', '', '', '', '', '', '', '', '', '', ''],
        ...
        ['', '', '', '', '', '', '', '', '', '', '', ''],
        ['', '', '', '', '', '', '', '', '', '', '', ''],
        ['신규등록 담당자', '', data.manager || '', '', '신규등록 담당자 연락처', '', data.contact || '', '', '신규등록 담당자 이메일', '', data.email || '', ''],
    ...
    dialogContent.innerHTML = `
        <div style="background: #f8f9fa; padding: 20px; border-radius: 10px; margin-bottom: 20px;">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px;">
                <div><strong>등록일자:</strong> ${data.reqDate || '-'}</div>
                <div><strong>등록번호:</strong> ${data.reqNo || '-'}</div>
                <div><strong>등록자:</strong> ${data.reqUser || '-'}</div>
                ...
                <table style="width: 100%; border-collapse: collapse;">
                    <thead>
                        <tr style="background: #0056b3; color: white;">
                            ...
                            <th style="padding: 10px; border: 1px solid #ddd;">상품코드</th>
                    ...
                    <tbody>
                        ...
                        `).join('') || '<tr><td colspan="12" style="padding: 20px; text-align: center; color: #6c757d;">등록된 품목이 없습니다.</td></tr>'}
                        <tr style="background: #e9ecef; font-weight: bold;">
                            <td colspan="7" style="padding: 10px; border: 1px solid #ddd; text-align: right;">합계</td>
        ...
        <div style="background: #fff3cd; padding: 20px; border-radius: 10px; margin-bottom: 20px;">
            <h4 style="margin-top: 0; color: #856404;">💼 담당자 정보</h4>
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px;">
...
async function exportSingleToExcel() {
    ...
    try {
        ...
        const buf = await workbook.xlsx.writeBuffer();
        saveAs(new Blob([buf], {type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"}), 
               `물품등록정보_${currentViewData.reqNo || 'Unknown'}.xlsx`);
    } catch (error) {
        console.error('엑셀 출력 오류:', error);
        alert('엑셀 출력 중 오류가 발생했습니다.');
    }
...
function deleteItem(savedName) {
    if (confirm('정말 이 물품등록정보를 삭제하시겠습니까?')) {
        const savedDataList = JSON.parse(localStorage.getItem('물품신규등록_저장목록') || '{}');
